stratSampling_EPIC <- function(target_IDs, EPICanno=EPICanno){	#stratified sampling based on DMPs. categolies are CpG island, CpG shore and non-CGI/non-shore
 ##Categolization of all probes into the categolies
  all.TSS.probes <- EPICanno[unique(c(grep("TSS200", EPICanno[,"UCSC_RefGene_Group"]), grep("5'UTR", EPICanno[,"UCSC_RefGene_Group"]))),"IlmnID"]#identification of all TSS probes
  all.CGI.probes <- EPICanno[grep("Island", EPICanno[!(EPICanno[,"IlmnID"] %in% all.TSS.probes),"Relation_to_UCSC_CpG_Island"]),"IlmnID"]#identification of all CGI probes
  all.shore.probes <- EPICanno[grep("Shore", EPICanno[!(EPICanno[,"IlmnID"] %in% all.TSS.probes),"Relation_to_UCSC_CpG_Island"]),"IlmnID"] #identification of all shore probes
  all.shelf.probes <- EPICanno[grep("Shelf", EPICanno[!(EPICanno[,"IlmnID"] %in% all.TSS.probes),"Relation_to_UCSC_CpG_Island"]),"IlmnID"]
  ...
  nall.shore.probes <- length(all.shelf.probes) #number of all shelf probes
  ...
  nshore.probes <- length(shelf.probes) #number of shore DMPs
  ...
  run.shore.probes <- all.shore.probes[floor(runif(nshore.probes, 1 ,nall.shore.probes))] #shelf probes
